...
//This Month Rev
//$thisMonthRev = "SELECT SUM(total) AS this_revenue " . ($role == 'Manager' ? "" : ", SUM(CASE WHEN user_code = '$activeuser' THEN total ELSE 0 END) AS this_user_revenue") . " FROM ledger WHERE MONTH(transaction_date) = MONTH(CURRENT_DATE()) AND YEAR(transaction_date) = YEAR(CURRENT_DATE())" . ($role == 'Manager' ? "" : " AND user_code = '$activeuser'");
$thisMonthRev = "SELECT SUM(total) AS this_revenue " . ($role == 'Manager' ? "" : ", SUM(CASE WHEN user_code = '$activeuser' THEN total ELSE 0 END) AS this_user_revenue") . " FROM ledger WHERE MONTH(transaction_date) = MONTH(CURRENT_DATE()) AND YEAR(transaction_date) = YEAR(CURRENT_DATE())" . ($role == 'Manager' ? "" : " AND user_code = '$activeuser'");

//Previous Month revenue
//$prevMonthRev = "SELECT SUM(total) AS prev_revenue " . ($role == 'Manager' ? "" : ", SUM(CASE WHEN user_code = '$activeuser' THEN total ELSE 0 END) AS prev_user_revenue") . " FROM ledger WHERE MONTH(transaction_date) = MONTH(CURRENT_DATE() - INTERVAL 1 MONTH) AND YEAR(transaction_date) = YEAR(CURRENT_DATE())" . ($role == 'Manager' ? "" : " AND user_code = '$activeuser'");
$prevMonthRev = "SELECT SUM(total) AS prev_revenue " . ($role == 'Manager' ? "" : ", SUM(CASE WHEN user_code = '$activeuser' THEN total ELSE 0 END) AS prev_user_revenue") . " FROM ledger WHERE MONTH(transaction_date) = MONTH(CURRENT_DATE() - INTERVAL 1 MONTH) AND YEAR(transaction_date) = YEAR(CURRENT_DATE())" . ($role == 'Manager' ? "" : " AND user_code = '$activeuser'");

//Last year revenue of this month 
//$lastYearRev = "SELECT SUM(total) AS last_revenue " . ($role == 'Manager' ? "" : ", SUM(CASE WHEN user_code = '$activeuser' THEN total ELSE 0 END) AS last_user_revenue") . " FROM ledger WHERE MONTH(transaction_date) = MONTH(CURRENT_DATE()) AND YEAR(transaction_date) = YEAR(CURRENT_DATE() - INTERVAL 1 YEAR)" . ($role == 'Manager' ? "" : " AND user_code = '$activeuser'");
$lastYearRev = "SELECT SUM(total) AS last_revenue " . ($role == 'Manager' ? "" : ", SUM(CASE WHEN user_code = '$activeuser' THEN total ELSE 0 END) AS last_user_revenue") . " FROM ledger WHERE MONTH(transaction_date) = MONTH(CURRENT_DATE()) AND YEAR(transaction_date) = YEAR(CURRENT_DATE() - INTERVAL 1 YEAR)" . ($role == 'Manager' ? "" : " AND user_code = '$activeuser'");
...
if ($rev_this != 'NONE') {
    if ($rev_this >= $base_revenue) {
        ...
        $MessageString = "This month has just started";
    }
    elseif ($rev_this < $base_revenue){
        $MessageString = "There is room for improvement! Keep working hard!";
    }
} else {
        ...
        <main>
            ...
            <div id="mainContent">
                <div id="greetingBox" class="greetingBox">
                    <p></p>
                    <h1>Happy here <?php echo date('l');?>, <?php echo $user; ?>!<?php if($role == 'Manager'): ?> Here's your clinic weekly update</h1>
                    <?php else: ?>
